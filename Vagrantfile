...
Vagrant.configure("2") do |config|
  ...
  #config.vm.provision :shell, :inline => "yes | yum -y update"
  #config.vm.provision :shell, :inline => "rpm -ivh http://yum.puppetlabs.com/puppetlabs-release-el-7.noarch.rpm"
  #config.vm.provision :shell, :inline => "yes | yum -y install puppet"
  ...
  #Tomcat installation
  #Copy tomcat service file to be able to run tomcat as a service
  config.vm.provision "file", source: "tomcat.service", destination: "tomcat.service"
  #Tomcat installation and its dependancies
  config.vm.provision "shell", inline: <<-SHELL
    yes | yum -y install wget
    echo "\n----- Installing Apache ------\n"
    yes | yum -y install httpd
    echo "\n----- Installing Tomcat ------\n"
    groupadd tomcat
    useradd -s /bin/false -g tomcat -d /opt/tomcat tomcat
    wget -q http://apache.mirrors.ionfish.org/tomcat/tomcat-8/v8.5.9/bin/apache-tomcat-8.5.9.tar.gz
    mkdir /opt/tomcat
